...
/* Initialize port status tracking if needed */
...
static int qca8337_config(struct phy_device *phydev)
{
	...
	if (qca8337_parse_config(phydev))
		return -EINVAL;
	...
	if (qca8337_sw_reset(phydev) < 0)
		return -1;

	mdelay(10);

	if (qca8337_hw_config(phydev) < 0)
		return -1;

	mdelay(10);

	if (qca8337_switch_init(phydev, swt_cfg) != 0) {
...
static int qca8337_startup(struct phy_device *phydev)
{
	...
	for (i = 0; i < swt_cfg->port_count; ++i) {
		...
		phy_data = phy_read(phydev, MDIO_DEVAD_NONE, S17_PHY_SPEC_STATUS);
		...
		if (need_phy_reset) {
			...
			qca8337_force_mac_1000M_full(phydev, port_id);
			/* Reset the PHY */
			phy_val = phy_read(phydev, MDIO_DEVAD_NONE, MII_BMCR);
			mdelay(100);
			phy_val |= BMCR_RESET;
			...
			/* Re-read PHY status after reset */
...
/*********************************************************************
 * FUNCTION DESCRIPTION: Initialize the QCA8337 PHY driver.
 * RETURN   : 0 on success, error code otherwise
 *********************************************************************/
